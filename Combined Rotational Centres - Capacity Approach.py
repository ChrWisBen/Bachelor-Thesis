...
Update per request:
- W₁ and W₄ labeled "W₁-CLT" and "W₄-CLT" and colored ORANGE.
- W₂ and W₃ labeled "W₂-RC" and "W₃-RC" and colored DARK TEAL.

Extras (same style as your previous script):
- Small dots for points (placement unchanged).
- Point labels placed with no-overlap logic vs labels + dots.
- Arrows from point label -> dot (labels not moved after placement).
- Wall labels offset from wall with arrow to wall centroid.
- W₂ forced to the right side of its wall.
- W₁ forced to the opposite side of its wall.
- Reference arrows wᵧ and wₓ in light teal:
    wᵧ from (0,5)->(0.5,5)
    wₓ from (5,0)->(5,0.5)
"""
...
def place_wall_label_with_arrow(ax, fig, wall, color,
    ...
    """
    Place wall label off the line, avoid overlaps, then draw arrow to wall centroid.
    Forcing logic uses the wall ID (W₁, W₂, ...), but displayed text
    uses WALL_DISPLAY_BY_NAME.
    """
    renderer = fig.canvas.get_renderer()
    ...
    # Force W₁ + W₂ to opposite/right side
    if name in ("W₁", "W₂"):
        side_order = [-1]
    else:
        side_order = [1, -1]
...
def plot_combined():
    ...
    ax.set_title("Rotational Centres, Hybrid Approach, Variation 1",
                 fontsize=13, pad=10)
    ...
    # Wall labels offset with arrows (color follows wall)
    for w in walls:
        name = w[0]
        color = WALL_COLOR_BY_NAME.get(name, TEAL_MID)
    ...
    ax.text(0.25, 5.2, "", ha="center", va="bottom",
    ...
    ax.text(5.2, 0.25, "wᵧ", ha="left", va="center",
            fontsize=11, color=TEAL_MID)

    ax.grid(True)
    plt.tight_layout()
    plt.savefig("rotational_centres_Hybrid_Alternate_Capacity.svg", format="svg")
    plt.show()
