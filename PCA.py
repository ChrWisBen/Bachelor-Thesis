...
- Correlation heatmap:
    * REVERSED colors (+1 teal, -1 orange)
    * 0 correlation = white midpoint
    ...
    * SAVED AS SVG
- PCA cumulative explained variance plot (TEAL) SAVED AS SVG
- PC1 vs PC2 scatter SAVED AS SVG
...
# Settings
# ---------------------------
excel_path = Path(r"C:\Users\ChristianWBendtsen\OneDrive - ABC\Skrivebord\Bachelor\RC_Wall_Cap_Samples\Cap_Rc_Norm.xlsx")
sheet_name = 0
...
heatmap_svg   = out_dir / "corr_heatmap.svg"
variance_svg  = out_dir / "pca_variance_retention.svg"
pc12_svg      = out_dir / "pc1_vs_pc2.svg"
...
# ---------------------------
# 5) Correlation heatmap (YOUR COLORS, REVERSED) + boosted low values
#    +1 teal, -1 orange, 0 white
#    SAVED AS SVG
...
if showplots:
    ...
    hm = sns.heatmap(
        ...
        annot=False,
        square=True,
        cbar_kws={"label": "Correlation (−1 to 1), SymLog scaled"},
        ax=ax
    )
    ...
    ax.set_title("Correlation Heatmap of Input Features - RC Wall Shear Capacity", fontsize=14)
    fig.tight_layout()
...
if showplots:
    ...
    ax2.set_title('PCA Variance Retention - RC Wall Shear Capacity')
    ax2.set_ylim(0, 1.02)
